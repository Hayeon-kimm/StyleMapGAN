...
class PixelNorm(nn.Module): #stylegan, forces the empirical standard deviation of the latent vector to be one.
    def __init__(self):
        super().__init__()

    def forward(self, input):
        return input * torch.rsqrt(torch.mean(input ** 2, dim=1, keepdim=True) + 1e-8) #1e-8=epsilon
        #rsqrt = 1 / (input)**(1/2)
...
class Generator(nn.Module):
    def __init__(
        ...
        for i in range(mapping_layer_num):
            if i != (mapping_layer_num - 1): #except for final layer
                in_channel = style_dim
                out_channel = style_dim
            else: #final layer
                in_channel = style_dim
                out_channel = style_dim * latent_spatial_size * latent_spatial_size #reshape
        ...
        self.mapping_z = nn.Sequential(*layers) #w
